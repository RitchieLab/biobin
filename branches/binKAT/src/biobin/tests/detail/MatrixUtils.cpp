...
void MatrixUtils::getColinearSet(const gsl_matrix* X, vector<unsigned int>& idx_permu){
	idx_permu.clear();
...
unsigned int MatrixUtils::checkColinear(const gsl_matrix* X, gsl_permutation* &P){
...
void MatrixUtils::setPermutation(const vector<unsigned int>& idx_permu, gsl_permutation* permu){
...
void MatrixUtils::applyInversePermutation(gsl_matrix* mat, const gsl_permutation* permu, bool byCol){
	// view_fn will give us a vector to permute
	gsl_vector_view (*view_fn)(gsl_matrix *, size_t) = byCol ? gsl_matrix_row : gsl_matrix_column;
	size_t iter = byCol ? mat->size1 : mat->size2;
