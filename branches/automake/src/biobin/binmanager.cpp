 ...
 * information for
 */
std::pair<uint, uint> BinManager::InitBins(std::map<uint, Knowledge::GroupManagerDB> &groups, 
	...
	while (meta != metaEnd) {
		...
		CollectGroupLeaves(gmgr, regionLookup, regionToBinnable, leaves, genesUsed);
		meta++;
	}
	...
	if (ExpandByExons) {
		std::map<uint, Utility::IdCollection>::iterator itr		= regionBins.begin();
		...
		while (itr != end) {
			...
			if (exons.size() > 0 || introns.size() > 0 || regulatory.size() > 0) {
				if (exons.size() > 0)
	...
	return std::make_pair(binID + 1, genotypeMap.size());

}

void BinManager::GenerateIntergenicBins(std::map<uint, std::map<uint, Utility::IdCollection> >& intergenic,  
					std::vector<uint>& indexLookup, 
					uint& binID) {
	std::map<uint, std::map<uint, Utility::IdCollection> >::iterator chrom = intergenic.begin();
	std::map<uint, std::map<uint, Utility::IdCollection> >::iterator chromEnd = intergenic.end();
	
	while (chrom != chromEnd) {
		std::map<uint, Utility::IdCollection>::iterator seg = chrom->second.begin();
		std::map<uint, Utility::IdCollection>::iterator segEnd = chrom->second.end();
		std::string chromosome = Utility::ChromFromIntChr(chrom->first);
		while (seg != segEnd) {
			GenerateBin(seg->second, indexLookup, binID, chromosome + "-" + Utility::ToString(seg->first));
			seg++;
		}
		chrom++;
	}
...
void BinManager::GenerateBins(std::map<Knowledge::Group*, Utility::IdCollection>& groups, 
					std::map<uint, std::set<uint> > binnables, 
					std::vector<uint>& indexLookup, 
					uint &binID) {
	std::map<Knowledge::Group*, Utility::IdCollection>::iterator itr = groups.begin();
	std::map<Knowledge::Group*, Utility::IdCollection>::iterator end = groups.end();
...
void BinManager::CollectGroupLeaves(Knowledge::GroupManagerDB& gmgr,
	...
	if (visited.find(groupIdx) == visited.end()) {
		...
		while (geneIdx != geneEnd) {
			snps.insert(regionToBinnable[*geneIdx].begin(), regionToBinnable[*geneIdx].end());
			geneIdx++;
		}
		...
		if (snps.size() > BinTraverseThreshold && group.groups.size() > 0) {
			...
			while (groupItr != groupEnd) 
				CollectGroupLeaves(gmgr, regionLookup, regionToBinnable, leaves, genesUsed, visited, *groupItr++);
		} else {
...
void BinManager::CollectGroupLeaves(Knowledge::GroupManagerDB& gmgr,
	...
	while (itr != end) 
		CollectGroupLeaves(gmgr, regionLookup, regionToBinnable,
			leaves, genesUsed, visited, *itr++);
...
std::set<uint> BinManager::ParseSNP(uint snpIndex, std::vector<char>& genotypes, std::vector<Individual>& data) {
	...
	std::multimap<uint, uint>::iterator multiStart = multiBins.lower_bound(snpIndex);
	std::multimap<uint, uint>::iterator multiStop = multiBins.upper_bound(snpIndex);
...
void BinManager::DescribeLocus(uint snpIndex, std::ostream& os, Knowledge::RegionManagerDB& regions, Knowledge::SnpDataset& snps) {
	...
	if (binIDs[snpIndex] != (uint)-1) {
		if (multiBins.find(snpIndex) == multiBins.end())
			...
			std::multimap<uint, uint>::iterator itr = multiBins.lower_bound(snpIndex);
			std::multimap<uint, uint>::iterator end = multiBins.upper_bound(snpIndex);
			while (itr != end) {
	...
	if (isRare) 
		os<<"Rare ";
	os<<"Variant,"<<Utility::Join(regionNames, ":")<<","<<Utility::Join(bins, ":")<<"\n";
