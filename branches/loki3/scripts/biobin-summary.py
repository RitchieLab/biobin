...
if __name__ == "__main__":
	...
	for fn in glob.glob(args.prefix + "-*bins.csv"):
		...
		if not header_printed:
			header_printed = True
			print args.output_sep.join(["Phenotype", "Bin", "Num_Cases", "Num_Controls", "Num_Loci", "Num_Vars", "Case_Vars", "Control_Vars"] + [n for n in pv_names])
		
		# Now, enumerate all the lists we have - they should have the same # of elements
		for i in xrange(len(bin_names)):
			print args.output_sep.join([pheno_name, bin_names[i], n_case, n_control, bin_loci[i], bin_vars[i], case_vars[i], ctl_vars[i]] + [pv_dict[n][i] for n in pv_names])
