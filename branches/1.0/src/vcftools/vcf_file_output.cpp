...
void vcf_file::output_per_site_nucleotide_diversity(const string &output_file_prefix)
{
	...
	for (unsigned int s=0; s<N_entries; s++)
	{
		...
		double pi=0.0;
		for (unsigned int ui=0; ui<(N_indv-1); ui++)
		{
			if (include_indv[ui] == false)
				continue;
			if (include_genotype[s][ui] == false)
			...
			for (unsigned int uj=0; uj<N_indv; uj++)
			{
				...
				if ((genotype2.first != -1) && (genotype2.second != -1))
				{
					if (genotype2.first != genotype2.second)
						pi += 1.0;
					n_comparisons++;
				}

				if ((genotype1.first != -1) && (genotype2.first != -1))
				{
					if (genotype1.first != genotype2.first)
						pi += 1.0;
					n_comparisons++;
				}

				if ((genotype1.first != -1) && (genotype2.second != -1))
				{
					if (genotype1.first != genotype2.second)
						pi += 1.0;
					n_comparisons++;
				}

				if ((genotype1.second != -1) && (genotype2.first != -1))
				{
					if (genotype1.second != genotype2.first)
						pi += 1.0;
					n_comparisons++;
				}

				if ((genotype1.second != -1) && (genotype2.second != -1))
				{
					if (genotype1.second != genotype2.second)
						pi += 1.0;
					n_comparisons++;
				}
			}
...
void vcf_file::output_PCA(const string &output_file_prefix)
{
	error("Support for PCA removed as LAPACK doesn't appear to be standard.");
	cout << output_file_prefix << endl;
	/*
	...
	printLOG("Outputting Principal Component Analysis\n");
