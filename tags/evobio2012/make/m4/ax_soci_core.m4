...
if test "x$want_soci" = "xyes"; then
    ...
    if test "$ac_soci_path" != ""; then
        ...
        if test "x$include_succeed" != "xyes"; then
               ...
               for ac_soci_path_tmp in `echo $CPLUS_INCLUDE_PATH | sed 's/:/ /g' | sed 's/\/include\/* */ /g'` "$std_paths" ; do
        ...
        if test "$ac_soci_path" != ""; then
            ...
            AC_LINK_IFELSE(
              ...
              if test "x$link_succeed" != "xyes"; then
                  ...
                  for ac_soci_path_tmp in `echo $LPATH | sed 's/:/ /g' | sed 's/\/lib\(64\)*\/* */ /g'` "$std_paths" ; do
                    for libsubdir in $libsubdirs ; do
