...
SVN_REPO="https://svn.ritchielab.psu.edu/svn"
...
mkdir build
cd build
../configure --prefix=$INST_DIR/common --exec-prefix=$INST_DIR/$RITCHIELAB_OS --datadir=$RITCHIELAB_GROUP_DIR/datasets/loki --disable-loki --enable-debug=info >/dev/null 2>&1
...
if test $(make -j10 distcheck 2>&1 | tail -4 | grep -c "ready for distribution") -lt 1; then
	...
	{ echo "$PROJECT build failed on $(hostname -A)"; autoreconf -i .. 2>&1 ; ../configure --prefix=$INST_DIR/common --exec-prefix=$INST_DIR/$RITCHIELAB_OS --disable-loki --enable-debug=info 2>&1 ; make distcheck 2>&1 ; echo "Sent from $(hostname -A)" ; } | mail -s "$PROJECT nightly build failed on $(hostname -A)" -r software@ritchielab.psu.edu software@ritchielab.psu.edu
else
