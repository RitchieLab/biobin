...
if test "x$want_soci_sqlite" = "xyes"; then
	...
	if test "x$have_sqlite" = "xno"; then		   
		PKG_CHECK_MODULES([SQLITE],[sqlite3 >= 3.3], 
			[have_sqlite=yes; AC_DEFINE([HAVE_SQLITE],[],[SQLite is available])],
			[have_sqlite=no; AC_DEFINE([NO_SQLITE],[],[No SQLite library available])])
	fi
	
	# Only bother if we actually HAVE SQLite, o/w it's pointless!
	if test "x$have_sqlite" = "xyes"; then
		...
		if test "$ac_soci_sqlite_path" != ""; then
			...
			if test "$SOCI_CPPFLAGS" != ""; then
				...
				if test -r "$ac_soci_incl_dir/sqlite3/soci-sqlite3.h"; then
					SOCI_SQLITE_CPPFLAGS="-I$ac_soci_incl_dir/sqlite3"
			...
			CPPFLAGS="$CPPFLAGS_SAVED $SQLITE_CPPFLAGS $SOCI_CPPFLAGS $SOCI_SQLITE_CPPFLAGS"
			export CPPFLAGS
			...
			if test "x$include_succeed" != "xyes"; then
			   ...
			   for ac_soci_path_tmp in $PATH_LIST; do
			   		if test -r "$ac_soci_sqlite_path_tmp/include/soci/sqlite3/soci-sqlite3.h"; then
	   					...
	   					break;
					elif test -r "$ac_soci_sqlite_path_tmp/include/soci/soci-sqlite3.h"; then
	   					SOCI_SQLITE_CPPFLAGS="-I$ac_soci_sqlite_path_tmp/include/soci"
				   		break;
		...
		if test "$ac_soci_sqlite_lib_path" != ""; then
			...
			# Check for SQLITE relative to sqlite path
			if test "$ac_soci_sqlite_path" != ""; then
				...
				LDFLAGS="$LDFLAGS_SAVED $SQLITE_LDFLAGS $SOCI_LDFLAGS $SOCI_LIB $SOCI_SQLITE_LDFLAGS $SOCI_SQLITE_LIB"
				export LDFLAGS
