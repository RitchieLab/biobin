...
int main(int argc, char *argv[])
{
	...
	vcf_file vcf(params.vcf_filename, params.vcf_compressed, params.chr_to_keep, params.chr_to_exclude);
	...
	printLOG("Kept " + int2str(N_indv) + " out of " + int2str(vcf.N_indv) + " Individuals\n");
	printLOG("Kept " + int2str(N_sites) + " out of " + int2str(vcf.N_entries) + " Sites\n");
	if (N_sites == 0)
		error("No data left for analysis!");

	if (params.diff_file != "")
	{	// Merge files - cannot be run with other output options.
		vcf_file vcf_diff(params.diff_file, params.diff_file_compressed, params.chr_to_keep, params.chr_to_exclude);
	...
	if (params.recode == true) vcf.print(params.output_prefix, params.INFO_to_keep, params.keep_all_INFO);
	...
	if (params.output_PCA == true) vcf.output_PCA(params.output_prefix);
