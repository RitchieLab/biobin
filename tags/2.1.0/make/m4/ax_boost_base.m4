...
if test "x$want_boost" = "xyes"; then
	...
	if test "x$succeeded" = "xyes"; then 
		...
		# Now, get the version!
		V=`cat "$ac_boost_path/include/boost/version.hpp" | grep BOOST_VERSION | grep '#define' | grep -v HPP | sed 's/.*BOOST_VERSION//g'`
		MAJ_V=`expr $V / 100000`
		...
		if test "$ac_boost_lib_path" != ""; then
			...
			for libsubdir in $libsubdirs; do			
				if ls "$ac_boost_path/$libsubdir/libboost_"*$MAJ_V.$MIN_V.$PATCH_V >/dev/null 2>&1; then
