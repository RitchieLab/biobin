...
void parameters::read_parameters()
{
	...
	while (i<argv.size())
	{
		...
		else if (in_str == "--keep-INFO-all") { keep_all_INFO=true; }		// Specify INFO to keep when recoding
		else if (in_str == "--keep-INFO") { INFO_to_keep.insert(get_arg(i+1)); i++; }		// Specify INFO to keep when recoding
		...
		else if (in_str == "--ldhat-geno") { output_as_ldhat_unphased = true; max_alleles=2; }
		else if (in_str == "--ldhat") { output_as_ldhat_phased = true; phased_only = true; max_alleles=2; } // Output as LDhat format
		else if (in_str == "--LROH") {output_LROH = true;}
		else if (in_str == "--maf") { maf = atof(get_arg(i+1).c_str()); i++; }								// Minimum Site MAF
		...
		else if (in_str == "--non-ref-af") { non_ref_af = atof(get_arg(i+1).c_str()); i++; }				// Minimum Site non-ref AF
		...
		else if (in_str == "--window-pi") { pi_window_size = atoi(get_arg(i+1).c_str()); i++; phased_only = true; }
		else
...
void parameters::print_params()
{
	...
	if (fst_file != defaults.fst_file)
	{
		if (vcf_compressed == false)
			...
			printLOG("\tEXPERIMENTAL!!! --gzfst " + fst_file + "\n");
	}
	...
	if (keep_all_INFO == true) printLOG("\t--keep-INFO-all\n");
	...
	if (maf != defaults.maf) printLOG("\t--maf " + dbl2str(maf, 3) + "\n");
	...
	if (non_ref_af != defaults.non_ref_af) printLOG("\t--non-ref-af " + dbl2str(non_ref_af, 3) + "\n");
	...
	if (output_PCA != defaults.output_PCA) printLOG("\t--pca\n");
	...
	if (vcf_filename != defaults.vcf_filename)
	{
		if (vcf_compressed == false)
			...
			printLOG("\tEXPERIMENTAL AND UNTESTED!!! --gzvcf " + vcf_filename + "\n");
	}
	...
	if (diff_file != defaults.diff_file)
	{
		if (vcf_compressed == false)
			...
			printLOG("\t--EXPERIMENTAL!! gzdiff " + diff_file + "\n");
	...
	if (INFO_to_keep.size() > 0)
		for (set<string>::iterator it=INFO_to_keep.begin(); it != INFO_to_keep.end(); ++it)
		{
			string tmp = *it;
...
void parameters::check_parameters()
{
	...
	if (max_maf < maf) error("Maximum MAF must be less that Minimum MAF.", 4);
	if (maf != defaults.maf)
	{
		if ((maf < 0.0) || (maf > 1.0)) error("MAF must be between 0 and 1.", 4);
	}
	if (max_maf != defaults.max_maf)
	{
		if ((max_maf < 0.0) || (max_maf > 1.0)) error("Maximum MAF must be between 0 and 1.", 4);
	}
	if (non_ref_af != defaults.non_ref_af)
	{
		if ((non_ref_af < 0.0) || (non_ref_af > 1.0)) error("Non-Ref Allele Frequency must be between 0 and 1.", 4);
	}
	if (max_non_ref_af < non_ref_af) error("Maximum Non-Ref Allele Frequency must be less that Minimum MAF.", 4);
