...
public:
	...
	bool GetIntegers(const char *key, vector<uint>& lines);
	...
	Utility::StringArray GetKeys() { return keys; }

	void Write(ostream& os);
	string comments;
protected:
	map<string, Utility::StringArray > strings;
...
public:
	...
	void InitSingletary(const char *key, const char *defValue, const char *comment);
	/**
...
protected:
	...
	std::set<string> singletary;		///< Fields that can only be a single value (thus won't ever be split or joined)
};
...
inline
bool FileToMap::GetIntegers(const char *key, vector<uint>& lines) {
	...
	if (itr != end) {
		isFound = true;
		vector<string>::iterator litr = itr->second.begin();
		vector<string>::iterator lend = itr->second.end();
		while (litr!=lend) {
			lines.push_back(atoi(litr++->c_str()));
		}
	}
	return isFound;
}
...
void FileToMap::SetValue(const char *key, const string& val) {
	if (strings.find(key) != strings.end()) 
		strings[key] = Utility::StringArray();
	vector<string>& list = strings[key];
	list.clear();
	list.push_back(val);
}
...
void FileToMapExplicit::InitKey(const char *key, const char *defValue, const char *comment) {
	string line = string(key) + string(" ") + ENV(key, defValue);
...
inline
void FileToMapExplicit::InitSingletary(const char *key, const char *defValue, const char *comment) {
	singletary.insert(key);
	string line = string(key) + string(" ") + ENV(key, defValue);
	...
	strings[key] = StringArray();
	strings[key].push_back(ENV(key, defValue));
	defaultValue[key]=true;
	varComments[key] = comment;
...
bool FileToMapExplicit::ParseLine(const char *line, uint val) {
	...
	if (valid) {
		...
		if (singletary.find(key) != singletary.end()) {
			string nLine = line;
			boost::algorithm::replace_first(nLine, key, "");
			strings[key].clear();
			strings[key].push_back(boost::algorithm::trim_left_copy(nLine));
		}
	}
	return valid;
}
