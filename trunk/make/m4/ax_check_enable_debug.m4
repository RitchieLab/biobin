...
AC_DEFUN([AX_CHECK_ENABLE_DEBUG],[
    ...
    if test "x$enable_debug" = "xyes" || test "x$enable_debug" = "x"; then
        ...
        dnl In addition to the optimization, disable range checking in the GSL
        dnl Also, disable assertions.
        if test "x${CFLAGS+set}" != "xset"; then
            CFLAGS="-O3 -DGSL_RANGE_CHECK_OFF -DNDEBUG"
        fi
        if test "x${CXXFLAGS+set}" != "xset"; then
            CXXFLAGS="-O3 -DGSL_RANGE_CHECK_OFF -DNDEBUG"
        fi
        ...
        dnl Do this at the command line so EVERY source gets it!
        dnl AC_DEFINE(NDEBUG,,[Define if debugging is disabled])
    fi
